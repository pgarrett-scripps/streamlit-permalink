...
with st.echo('Form 1'):
    ...
    with stp.form('form'):
        ...
        text_input_form1 = stp.text_input(label='text_input_form1', 
                                    value='xxx', 
                                    max_chars=25)
...
with st.echo('Selectbox'):
    selectbox = stp.selectbox(label='selectbox', 
                              options=OPTIONS)
...
with st.echo('Multiselect'):
    multiselect = stp.multiselect(label='multiselect', 
                                  options=OPTIONS, 
                                  default=['Option A', 1])
...
with st.echo('Number Sliders'):
    ...
    single_slider = stp.slider(label='single_slider', 
                               min_value=1, 
                               max_value=100, 
                               value=33)
    ...
    multi_slider = stp.slider(label='multi_slider', 
                              min_value=1, 
                              max_value=100, 
                              value=[42, 67])
...
with st.echo('Date Sliders'):
    ...
    single_date_slider = stp.slider(label='single_date_slider', 
    ...
    multi_date_slider = stp.slider(label='multi_date_slider', 
                                   min_value=date(2024, 1, 1), 
                                   max_value=date(2024, 12, 31), 
...
with st.echo('Time Sliders'):
    ...
    single_time_slider = stp.slider(label='single_time_slider', 
    ...
    multi_time_slider = stp.slider(label='multi_time_slider', 
                                   min_value=time(0, 0, 0), 
                                   max_value=time(23, 59, 59), 
...
with st.echo('Select Sliders'):
    ...
    select_slider = stp.select_slider(label='single_select_slider', 
                                      options=OPTIONS, 
                                      value=1)
    st.caption(f'select_slider: {select_slider}')
    st.caption(f'query_params: {st.query_params.get_all("single_select_slider")}')

    range_select_slider = stp.select_slider(label='range_select_slider', 
                                            options=OPTIONS, 
                                            value=["Option A", 2])
...
with st.echo('Text Input'):
    text_input = stp.text_input(label='text_input', 
                                value='xxx', 
                                max_chars=25)
...
with st.echo('Number Input'):
    number_input = stp.number_input(label='number_input', 
                                    min_value=1, 
                                    max_value=100, 
                                    value=42)
...
with st.echo('Text Area'):
    ...
    text_area_compress = stp.text_area(label='text_area_compress', 
                                       compress=True)
    st.caption(f'text_area_compress: {text_area_compress}')
    st.caption(f'query_params: {st.query_params.get_all("text_area_compress")}')

    text_area_compress_custom = stp.text_area(label='text_area_compress_custom', 
                                              compress=True, 
...
with st.echo('Date Input'):
    ...
    multi_date_input = stp.date_input(label='multi_date_input', 
                                      value=[date(2024, 1, 1), date(2024, 12, 31)])
...
with st.echo('Color Picker'):
    color_picker = stp.color_picker(label='color_picker', 
                                    value='#00EEFF')
...
if hasattr(st, 'segmented_control'):
    ...
    with st.echo('Segmented Control'):
        ...
        st.caption(f'query_params: {st.query_params.get_all("segmented_control_single")}')
