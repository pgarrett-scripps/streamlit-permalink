...
class HandlerSelectSlider(HandleWidget):
    ...
    def update_bound_args(self) -> None:
        ...
        if self.is_range_slider:
            ...
            if invalid_values:
                self.raise_url_error(f"Invalid values: {invalid_values}. Expected one of: {self.str_options}")
            ...
            if start_idx > end_idx:
                self.raise_url_error(f"Invalid range for select slider: start value '{actual_values[0]}' comes after end value '{actual_values[1]}' in options.")

            self.bound_args.arguments["value"] = actual_values

        else:
            str_value: str = self.validate_single_url_value(allow_none=False)

            if str_value not in self.str_options:
                self.raise_url_error(f"Invalid value: {str_value}. Expected one of: {self.str_options}")
