...
def validate_bool_url_value(url_key: str, url_value: str, handler_name: str) -> bool:
    ...
    if url_value not in TRUE_FALSE_VALUE:
        raise UrlParamError(f"Invalid value for {handler_name} parameter '{url_key}': {url_value}. Expected a {TRUE_FALSE_VALUE}.")
...
def validate_color_url_value(url_key: str, url_value: str, handler_name: str) -> str:
    if not re.match(r'^#([0-9a-fA-F]{6})$', url_value):
        raise UrlParamError(f"Invalid value for {handler_name} parameter '{url_key}': {url_value}. Expected a valid hex color code (e.g., #RRGGBB).")
    return url_value
...
def validate_date_url_value(url_key: str, url_value: str, handler_name: str) -> str:
    try:
        ...
        raise UrlParamError(f"Invalid value for {handler_name} parameter '{url_key}': {url_value}. Expected a valid date (e.g., 2023-01-01).")
...
def _validate_multi_options(options: Iterable[Any], widget_name: str) -> List[str]:
    ...
    """
    if options is None:
        raise ValueError(f"{widget_name.capitalize()} options cannot be None. Expected a non-empty list of options.")
    
    if not isinstance(options, Iterable):
        raise ValueError(f"Invalid value for {widget_name} options: {options}. Expected an iterable.")
    
    if len(options) == 0:
        raise ValueError(f"{widget_name.capitalize()} options cannot be empty. Expected a non-empty list of options.")
    ...
    if len(unique_options) != len(unique_str_options):
        raise ValueError(f"{widget_name.capitalize()} options must be unique when cast to strings. "
                         f"Options: {options}, "
    ...
    if len(set(map(StringHashableValue, options))) != len(options):
        # This warning message should mention that passing options through the url params may not result in the correct values since if there are two values which will equate to the sama value like 0 and false, then the first will be slected.
        # this message should be different than the one above
        warnings.warn(
...
def _validate_multi_default(default: Union[List[Any], Any, None], options: Union[List[Any], Any, None], widget_name: str) -> List[str]:    
    ...
    if invalid_defaults:
        raise ValueError(f"Invalid default values for {widget_name}: {invalid_defaults}. "
